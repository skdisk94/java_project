	...
	<div class="row" style="margin-top: 90px">
		...
		<div class="col-md-10">
			<div class="panel panel-primary" align="center">
				<div class="panel-heading">고객별 매출액</div>
				<!-- <form action="order 와 sold_product에 인서트를 하기위한 컨트롤로" method="post"> -->
				<form action="../control/daily_control.jsp?action=detail" method="post">
					<table class="table">
						<thead>
							<th class="col-md-1 text-center">고객Id</th>
						...
						<c:set var="orderByCustomer" value="${requestScope.orderByCustomer}" />
						<c:forEach var="odDto" items="${orderByCustomer}">

							<tr>
							</tr>
							<c:set var="detailList" value="${requestScope.detailList}" />
								<c:forEach var="dDto" items="${detailList}" varStatus="status">
									<c:choose>
										<c:when test="${odDto.oCustomerId.equals(dDto.customerId)&&!odDto.oCustomerId.equals(detailList[status.index-1].customerId)}">
										<tr>
										<td align="center">${odDto.oCustomerId}</td>
										<td align="center">${dDto.date }</td>
										<td align="center">${dDto.orderId }</td>
										<td align="center">${dDto.pName }</td>
										<td align="center">${dDto.unitPrice }</td>
										<td align="center">${dDto.quantity }</td>
										<td align="center">${dDto.sumPrice }</td>
										<td align="center">${odDto.sumPrice}</td>
										</tr>
										</c:when>
										<c:when test="${odDto.oCustomerId.equals(dDto.customerId)}">
										<tr>
										<td align="center" style="border-top : hidden"></td>
										<td align="center" style="border-top : hidden">${dDto.date }</td>
										<td align="center" style="border-top : hidden">${dDto.orderId }</td>
										<td align="center" style="border-top : hidden">${dDto.pName }</td>
										<td align="center" style="border-top : hidden">${dDto.unitPrice }</td>
										<td align="center" style="border-top : hidden">${dDto.quantity }</td>
										<td align="center" style="border-top : hidden">${dDto.sumPrice }</td>
										<td align="center" style="border-top : hidden"></td>
										</tr>
										</c:when>
									</c:choose>
							...
							<td colspan="8" align="center">
								<nav>
									<ul class="pagination">
										...
										<c:set var="pageArrayByCustomer" value="${requestScope.pageArrayByCustomer}" />
										<c:forEach var="pageNum" items="${pageArrayByCustomer}">
											<c:choose>
												...
												<c:otherwise>
													<li><a
														href="../control/admin_control.jsp?action=viewCustomer&page=${pageNum}">${pageNum}</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>
